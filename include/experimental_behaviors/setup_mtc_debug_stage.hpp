...
/**
 * @brief Example MTC behavior using the new MTCStageHelper template
 * 
 * This demonstrates how to use the helper to create a rotate behavior with minimal boilerplate.
 * Compare this to the existing SetupMTCRotateAroundFrameAxis to see the reduction in code.
 */
...
public:
  ...
  /**
   * @brief Provide ports specific to this behavior
   * The helper automatically includes common ports (task, planning_group, scaling, etc.)
   */
  static BT::PortsList providedPorts()
  {
    return MTCCommonPorts::mergePorts({
  ...
  /**
   * @brief Create and configure the MTC stage
   * 
   * This is the only method that needs custom implementation - the helper handles everything else!
   * The StageInputs contains common validated inputs: task, planning_group_name, velocity_scale, etc.
   */
  ...
  createStage(const StageInputs& inputs)
  {
    ...
    // Validate frames using helper utilities
    ...
    // Create cartesian planner (you would use your preferred planner here)
    auto planner = std::make_shared<moveit::task_constructor::solvers::CartesianPath>();
    // Note: This is simplified - in practice you'd use MTCCartesianPlanner and configure scaling factors:
    // auto planner = std::make_shared<mtc_solvers::MTCCartesianPlanner>();
    // planner->setMaxVelocityScalingFactor(inputs.velocity_scale);
    // planner->setMaxAccelerationScalingFactor(inputs.acceleration_scale);
    // planner->setIgnoreEnvironmentCollisions(inputs.ignore_environment_collisions);

    // Create and configure the MTC stage  
